...
class InventoryManager:
    ...
    def get_all_products(self, search_term=None, category=None, purchase_location=None, sort_by='p.name', sort_order='ASC', page=None, per_page=None):
        """
        ...
        if category: # Assuming 'category' is the category name or ID. Let's assume name for now.
            # If category is ID, the condition should be p.category_id = ?
            # If category is name, the condition should be LOWER(c.name) = ?
            # The app.py list_products_view seems to pass category name.
            where_clauses.append("LOWER(c.name) = ?")
            params.append(category.lower())

        if purchase_location:
            where_clauses.append("LOWER(p.purchase_location) LIKE ?") # Using LIKE for flexibility
            params.append(f"%{purchase_location.lower()}%")

        if where_clauses:
            query_parts.append("WHERE " + " AND ".join(where_clauses))

        # Validate sort_by to prevent SQL injection if it's too dynamic
        allowed_sort_columns = ['p.id', 'p.name', 'c.name', 'p.unit_of_measure', 'p.default_expiry_days', 'p.par_level', 'p.max_holding_amount', 'p.purchase_location']
        if sort_by not in allowed_sort_columns:
            sort_by = 'p.name' # Default to product name if invalid sort_by
        ...
        if page is not None and per_page is not None:
            try:
                ...
                if page_num > 0 and per_page_num > 0:
                    offset = (page_num - 1) * per_page_num
                    query_parts.append("LIMIT ? OFFSET ?")
                    params.extend([per_page_num, offset])
            except ValueError:
                # Handle error or log if page/per_page are not valid integers
                print(f"Warning: Invalid page or per_page values: page={page}, per_page={per_page}")
                pass # Or raise an error, or default to no pagination

        final_query = " ".join(query_parts)

        try:
            products = self._execute_query(final_query, tuple(params), fetch_all=True)
