...
def make_recipe_view(recipe_name):
    ...
    if request.method == 'POST':
        add_ons_str = request.form.get('add_ons_hidden')
        if add_ons_str:
            import json
            try:
                add_ons = json.loads(add_ons_str)
                for addon in add_ons:
                    ingredients_to_consume_details.append({
                        'item_name': addon['item_name'],
                        'quantity_to_consume': float(addon['quantity_to_consume']),
                        'is_partial': False
                    })
...
def api_seed_garden():
    ...
    cat_id = None
    manager = get_manager()
    cats = manager.get_all_categories_with_subcategories()
    for c in cats:
        if c['name'] == 'Produce':
            cat_id = c['id']
            break
    ...
    for plant in plants:
        ...
        for p in all_prods:
            if p['name'].lower() == plant.lower():
                ...
                break
        if not prod_id:
            manager.create_product(name=plant, category_id=cat_id, subcategory_id=None, unit_of_measure='Eaches', default_expiry_days=7)
            all_prods = manager.get_all_products_export()
            for p in all_prods:
                if p['name'].lower() == plant.lower():
                    prod_id = p['id']
...
def api_seed_recipes():
    ...
    # Tacos
    tacos = recipe_mngr.add_recipe({"name": "Classic Beef Tacos", "description": "Brown beef, add seasoning. Warm shells.", "instructions": "", "output_yield": 1, "ingredients": []})
    tacos_id = tacos['id'] if tacos and tacos.get('success') else None
    if tacos_id:
    ...
    # Poutine
    poutine = recipe_mngr.add_recipe({"name": "Classic Poutine", "description": "Cook fries, warm sauce, add cheese curds.", "instructions": "", "output_yield": 1, "ingredients": []})
    poutine_id = poutine['id'] if poutine and poutine.get('success') else None
    if poutine_id:
    ...
    # Fajitas
    fajitas = recipe_mngr.add_recipe({"name": "Chicken Fajitas", "description": "Cook chicken, add peppers.", "instructions": "", "output_yield": 1, "ingredients": []})
    fajitas_id = fajitas['id'] if fajitas and fajitas.get('success') else None
    if fajitas_id:
...
@app.route('/receipt-scanner', methods=['GET', 'POST'])
@login_required
def receipt_scanner():
    parsed_items = []
    all_products = get_manager().get_all_products(page=None, per_page=None)
    aliases = get_manager().get_all_aliases()
    ...
    if request.method == 'POST':
        ...
        for line in raw_text.split('\n'):
            ...
            clean_text = re.sub(r'\$\d+\.\d+', '', original_text).strip()
    ...
    return render_template('receipt_scanner.html', parsed_items=parsed_items, all_products=all_products)
...
def receipt_confirm():
    ...
    # Iterate through form keys to find product_id_*
    items_to_add = []
    import re
    for key in request.form.keys():
        match = re.match(r'^product_id_(\d+)$', key)
        if match:
            idx = match.group(1)
            ignore = request.form.get(f'ignore_{idx}')
            if ignore == 'yes':
                continue
                
            product_id_str = request.form.get(f'product_id_{idx}')
    ...
    for product_id, qty in items_to_add:
        result = get_manager().add_inventory_item(product_id, qty, purchase_location=purchase_location)
        if result.get('success'):
            success_count += 1
            
    if success_count > 0:
        flash(f"Successfully added {success_count} items to inventory from receipt!", "success")
    else:
        flash("No valid items were selected to add.", "warning")
...
if __name__ == '__main__':
    ...
    app.config['DATABASE'] = os.path.join(instance_path, 'food_app.db')
