...
def log_purchase_view():
    if request.method == 'POST':
        ...
        if not quantity_purchased_str:
            ...
            try:
                quantity_purchased = float(quantity_purchased_str)
                if quantity_purchased <= 0:
                    errors.append("Quantity purchased must be a positive number.")
            except ValueError:
        ...
        if not cost_per_unit_str:
            ...
            try:
                ...
                if cost_per_unit < 0: # Cost can be 0, but not negative
                    errors.append("Cost per unit cannot be negative.")
            except ValueError:
                errors.append("Cost per unit must be a valid number.")

        # Vendor is optional, no specific validation here beyond stripping whitespace.

        if errors:
            for error in errors:
                flash(error, 'error')
            # Repopulate filters and products for the form
            search_name = request.args.get('search_name', '') # Preserve filters if any
            search_category = request.args.get('search_category', '')
            all_categories_list = manager.get_all_categories() # Fetch category names

            products_filter_args = {}
            if search_name: products_filter_args['search_term'] = search_name
            if search_category: products_filter_args['category'] = search_category
            products_filter_args['page'] = None # Fetch all for dropdown
            products_filter_args['per_page'] = None
            products_for_dropdown = manager.get_all_products(**products_filter_args)
        ...
        # If validation passes, call manager method
        result = manager.log_purchase(
            product_id=product_id,
            purchase_date_str=purchase_date_str,
            quantity_purchased=quantity_purchased,
            cost_per_unit=cost_per_unit,
            vendor=vendor if vendor else None # Pass None if vendor is empty
        )
        ...
        if result.get("success"):
            ...
            search_category = request.args.get('search_category', '')
            all_categories_list = manager.get_all_categories()

            products_filter_args = {}
            if search_name: products_filter_args['search_term'] = search_name
            if search_category: products_filter_args['category'] = search_category
            products_filter_args['page'] = None
            products_filter_args['per_page'] = None
            products_for_dropdown = manager.get_all_products(**products_filter_args)

            return render_template('log_purchase.html',
                                   form_data=request.form, # Repopulate with submitted data
                                   products_for_dropdown=products_for_dropdown,
                                   all_categories=all_categories_list,
                                   today_date_iso=date.today().isoformat())
    ...
    all_categories_list = manager.get_all_categories() # Returns list of category names

    products_filter_args = {}
    if search_name:
        products_filter_args['search_term'] = search_name
    if search_category:
        # manager.get_all_products expects category name for filtering by category.
        products_filter_args['category'] = search_category
    ...
    # Default form data for GET request (e.g., today's date)
    form_data_get = {'purchase_date': date.today().isoformat()}
    ...
    return render_template('log_purchase.html',
                           ...
                           all_categories=all_categories_list,
                           today_date_iso=date.today().isoformat(),
                           request_args=request.args) # Pass request.args for filters in template
